...
class ResNet_s(nn.Module):
    ...
    def __init__(self, block, num_blocks, num_experts, num_classes=10, reduce_dimension=False, layer2_output_dim=None, layer3_output_dim=None, use_norm=False, returns_feat=True, use_experts=None, s=30):
        ...
        self.groups = self.num_experts

        if layer2_output_dim is None:
            if reduce_dimension:
                layer2_output_dim = 24 * self.groups
            else:
                layer2_output_dim = 32 * self.groups

        if layer3_output_dim is None:
            if reduce_dimension:
                layer3_output_dim = 48 * self.groups
            else:
                layer3_output_dim = 64 * self.groups

        block_ = BasicBlock_
        self.in_planes = self.in_planes * self.groups
        self.layer2s = self._make_layer(block_, layer2_output_dim, num_blocks[1], stride=2, groups=self.groups)
        self.in_planes = self.next_in_planes
        self.layer3s = self._make_layer(block_, layer3_output_dim, num_blocks[2], stride=2, groups=self.groups)
        self.in_planes = self.next_in_planes
        
        if use_norm:
            self.linears = nn.ModuleList([NormedLinear(layer3_output_dim // self.groups, num_classes) for _ in range(num_experts)])
        else:
            self.linears = nn.ModuleList([nn.Linear(layer3_output_dim // self.groups, num_classes) for _ in range(num_experts)])
            s = 1
    ...
    def _make_layer(self, block, planes, num_blocks, stride, groups):
        ...
        for stride in strides:
            layers.append(block(self.next_in_planes, planes, stride, groups))
    ...
    def forward(self, x):
        ...
        out_ind = torch.split(out, int(out.shape[1]//self.num_experts), dim=1)

        for ind in use_experts:
            outs.append(self._separate_part(out_ind[ind], ind))
            weight_ = self.linears[ind].weight
            weights.append(F.normalize(weight_, dim=0))
        
        self.feat = torch.stack(self.feat, dim=1) # 特征, 在linear之前
